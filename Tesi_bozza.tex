...
\lfoot{Analisi mediante topic model}
...
Come più volte sottolineato, l'algoritmo si può adattare anche ad altri contesti nel campo dei social media: basta, ad esempio, modificare la sorgente dei dati acquisiti (nell'appendice finale si vedrà come estrarre anche dati dai social). Gli studi che si possono condurre sono svariati, con applicazioni che vanno dalla \emph{sentiment analysis} alle ricerche in campo sociologico. Esempi di applicazioni esistenti ne abbiamo già forniti. 

Un esempio rilevante di applicazioni già sviluppate che devono confrontarsi con l'enorme flusso di dati provenienti dalla rete e con la necessità di classificarli, è costituito dai  \emph{motori di ricerca}. Essi devono costantemente analizzare dati voluminosi dalla rete per renderli disponibili all'utenza. L'uso dei topic model in quest'ambito (specialmente quelli che verranno trattati in seguito) è estremamente diffuso. 
...
\section[Il linguaggio Python e le sue librerie]{Il linguaggio Python e le sue librerie
	\sectionmark{Python}}
\sectionmark{Python}
...
Con cui abbiamo creato un RDD. Il comando \emph{textFile} legge il file di testo, ovvero il corpo dell'articolo, all'indirizzo \emph{file\_path} specifico, e indicizza i termini con il comando \emph{zipWithIndex}, il quale associa ad ogni parola un numero (numeri uguali a parole uguali); infine, con la trasformazione \emph{map} si crea una matrice Term-Document e il comando \emph{createDataFrame} genera un dataframe a partire da \emph{data}. Successivamente abbiamo
...
\end{verbatim}
ovvero inizializziamo il \emph{CountVectorizer} che trasforma una collection di testo  in un vettore di \emph{token}  e lo applichiamo al nostro dataframe. Il risultato è il modello di trasformazione che occorre al dataframe, un passaggio intermedio che permette di generare il \emph{corpus}, che sarà l'input dell'LDA. Quest'ultimo si ottiene come RDD selezionando con un'altra trasformazione gli input di interesse.
...
% Come sono fatti questi oggetti???
...
\begin{figure}
\centering
\includegraphics[width=.9\textwidth]{Schema}
...
È importante considerare che alcune testate possiedono sezioni per diverse tematiche (cronaca sportiva, rosa, finanza), nel nostro caso ci siamo limitati allo scraping della pagina principale, ovvero della vetrina notizie che ogni giornale compone, supponendo trattarsi di quelle di maggior rilievo per quella testata.
...
Di esso accorperemo le voci \emph{title, text, tags, resume} a formare un'unica stringa di testo che rappresenterà il nostro documento da analizzare.
...
\item CPU: 1.80 GHz
\end{itemize}
Chiaramente le specifiche temporali sono relative al tipo supporto hardware che si utilizza, per cui, ad esempio, con il doppio o il quadruplo di \emph{core} con medesime caratteristiche, il tempo di esecuzione dimezza, diventa un quarto, ecc.
...
\begin{enumerate}
\item \emph{Fase di Scraping}: durante il periodo di raccolta, vengono prelevati i contenuti testuali dai siti di Feed RSS prescelti;
\item \emph{Fase di memorizzazione}: che va di pari passo allo scraping. Tutte le informazioni vengono salvate nel formato mostrato in precedenza e si presta attenzione ai possibili doppi estratti confrontando il titolo;
...
\item \emph{Fase di NLP}: in cui si applica una procedura di rimozione delle \emph{stopwords} e si esegue lo \emph{stemming} delle parole rimaste;
\item \emph{LDA con Spark}: in cui si richiama la console Spark e si esegue l'algoritmo LDA sugli articoli. Si definisce un corpus costituito dai topic estratti e si applica una nuova rimozione delle \emph{stopwords};
...
\item Quelle la cui parola originaria resta comunque dubbia, come \emph{stat}: scriveremo \emph{stat\textbf{o}}, ma se l'occorrenza si riferisca al concetto di Stato inteso come nazione o al participio passato del verbo essere non è dato saperlo. Purtroppo questo è un problema intrinseco della procedura che non potrà essere risolto in questa sede.
\end{enumerate}
In presenza di occorrenze molto numerose ignoreremo quelle sporadiche anche se il significato differisce (come nel caso di \emph{stat\textbf{o}}, a cui non abbiamo affiancato la parola \emph{statuto} perché compare una volta sola). A patto di sacrificare la visione del numero di occorrenze, che segnaleremo se degno di nota,  potremo in questo modo visualizzare per intero e senza difficoltà il risultato dell'analisi.

A questo punto dovremmo verificare che l'output del nostro sistema è effettivamente coerente: proviamo con un caso controllato con la collection di nome \emph{Sport} che raccoglie 50 articoli sportivi recenti relativi al campionato calcistico italiano (Serie A), in particolare degli ultimi eventi sportivi\footnote{Settimana dall'1 al 6 Gennaio 2018}. Dato il numero di articoli ridotto, solo in questo caso abbiamo 3 topic finali di 10 parole:

\begin{enumerate}
\item \textbf{Topic 1}:  port\textbf{a}, inter, spalletti, fiorentina, simeone, chiesa, icardi, allen\textbf{atore}, squadr\textbf{a}, ultim\textbf{e};
   
\item \textbf{Topic 2}:  chi\textbf{evo}, ser\textbf{ie}, udines\textbf{e}, radovanovic, tomovic, allen\textbf{atore}, vittor\textbf{ia}, oddo, part\textbf{ita}, napoli;
  

\item \textbf{Topic 3}: allen\textbf{atore}, grupp\textbf{o}, port\textbf{a}, segn\textbf{a}, lavor\textbf{o}, ultim\textbf{e}, meret, salvezz\textbf{a}, adess\textbf{o}, oddo.
\end{enumerate}
...
Statistiche\footnote{Per \emph{Totale parole} si intende il conteggio delle parole stemmizzate, ovvero il conteggio che viene effettuato quando gli articoli hanno già subito il processo di pulitura e di rimozione delle stopwords.} 
\begin{verbatim}
Totale articoli: 448 
...
\item \textbf{Topic 8}: person\textbf{e}, condizion\textbf{i}, migrant\textbf{i}, ricerc\textbf{a}, milion\textbf{i}, rom\textbf{a}, san, success\textbf{ivo}, unit\textbf{i}, rohingya, francesc\textbf{o}, sal\textbf{ute}/sal\textbf{uto};
...
L'output è il seguente:
\begin{enumerate}
\item \textbf{Topic 1}: stat\textbf{o},  part\textbf{e}/part\textbf{ito}/part\textbf{ire}/ part\textbf{ita}, lavor\textbf{o}, mil\textbf{ano}/mil\textbf{a}, donn\textbf{a}, ital\textbf{ia}, ragazz\textbf{i}, figl\textbf{io}, person\textbf{e}, polit\textbf{ica}, port\textbf{a}, parl\textbf{a}/parl\textbf{amento};


\item \textbf{Topic 2}: stat\textbf{o}, donn\textbf{a}, ragazz\textbf{i}, poliz\textbf{ia}, accus\textbf{a}, uom\textbf{o}, omicid\textbf{io}, figl\textbf{io}, procur\textbf{a}, ferm\textbf{ato}, carabinieri, uccis\textbf{o};

\item \textbf{Topic 3}: part\textbf{e}/part\textbf{ito}/part\textbf{ire}/ part\textbf{ita}, ital\textbf{ia}, polit\textbf{ica}, pd, renz\textbf{i}, parl\textbf{a}/parl\textbf{amento}, vot\textbf{o}, berluscon\textbf{i}, president\textbf{e}, ser\textbf{ie}/ser\textbf{a}, rom\textbf{a}, stat\textbf{o}


\item \textbf{Topic 4}: lavor\textbf{o}, part\textbf{e}/part\textbf{ito}/part\textbf{ita}, pd, govern\textbf{o}, renz\textbf{i}, pens\textbf{are}, legg\textbf{e}/legg\textbf{ere}, vot\textbf{o}, ministr\textbf{o}, stat\textbf{o}, polit\textbf{ica}, president\textbf{e};

  
\item \textbf{Topic 5}  mil\textbf{ano}/mil\textbf{a}, part\textbf{e}/part\textbf{ito}/part\textbf{ita}, lavor\textbf{o}, eur\textbf{o}, stat\textbf{o}, centr\textbf{o}, president\textbf{e}, novembr\textbf{e}, italian\textbf{a}, govern\textbf{o}, cost\textbf{a}, vend\textbf{ita};
   
	
\item \textbf{Topic 6}: ital\textbf{ia}, paes\textbf{e}, rom\textbf{a}, president\textbf{e}, stat\textbf{o}, ministr\textbf{o}, donn\textbf{a}, forz\textbf{a}/forz\textbf{e}, pubblic\textbf{o}/pubblic\textbf{a}, poss\textbf{ono}, success\textbf{o},  sub\textbf{ito};
   

\item \textbf{Topic 7}:  figl\textbf{io}, vit\textbf{a}, donn\textbf{a}, mil\textbf{ano}/mil\textbf{a}, parl\textbf{a}/parl\textbf{amento}, renz\textbf{i}, italian\textbf{a}, polit\textbf{ica}, berluscon\textbf{i}, vol\textbf{o}/vol\textbf{uto}, pd, numer\textbf{o};
   
   
\item \textbf{Topic 8}: parl\textbf{a}/parl\textbf{amento}, polit\textbf{ica}, renz\textbf{i}, berluscon\textbf{i}, dett\textbf{o}, vol\textbf{o}/vol\textbf{uto}, pens\textbf{a}, person\textbf{e}, nom\textbf{e}, pd, milion\textbf{i}, leader;


\item \textbf{Topic 9}:  accus\textbf{a}, ser\textbf{ie}/ser\textbf{a}, ragazz\textbf{i}, enne\footnote{Deriva dall'uso di scrivere, ad esempio, \emph{16enne} in luogo di \emph{sedicenne} e così via...}, poliz\textbf{ia}, ex, sessual\textbf{e}, figl\textbf{io}, giudic\textbf{e}, rapport\textbf{o}, iniz\textbf{o}, part\textbf{e}/part\textbf{ito}/part\textbf{ire}/part\textbf{ita};
   
\item \textbf{Topic 10}: ragazz\textbf{i}, donn\textbf{a}, trov\textbf{ato}, violenz\textbf{a}, giovan\textbf{i}, ital\textbf{ia}, parl\textbf{a}/parl\textbf{amento}, auto, polit\textbf{ica}, scuol\textbf{a}, ser\textbf{ie}/ser\textbf{a}, amic\textbf{i};


\item \textbf{Topic 11}: ser\textbf{ie}/ser\textbf{a}, vit\textbf{a}, stat\textbf{o}, mil\textbf{ano}/mil\textbf{a}, ultim\textbf{i}, napol\textbf{i}, italian\textbf{a}, classif\textbf{ica}, inter, pass\textbf{ato}/pass\textbf{o}, ben\textbf{e}, gioc\textbf{o};


\item \textbf{Topic 12}: scuol\textbf{a}, vol\textbf{o}/vol\textbf{uto}, ragazz\textbf{i}, ital\textbf{ia}, pubblic\textbf{o}/pubblic\textbf{a}, pens\textbf{are}, figl\textbf{io}, stat\textbf{o}, sent\textbf{ito}, già, prov\textbf{a}, insegn\textbf{ante}

   
\item \textbf{Topic 13}:  legg\textbf{e}/legg\textbf{ere}, figl\textbf{io}, president\textbf{e}, milion\textbf{i}, trov\textbf{o}, govern\textbf{o}, commission\textbf{e}, bilanc\textbf{io}, mort\textbf{e}, econom\textbf{ia}, parl\textbf{a}/parl\textbf{amento}, genitor\textbf{i};


\item \textbf{Topic 14}: vit\textbf{a}, legg\textbf{e}/legg\textbf{ere}, pass\textbf{ato}, milion\textbf{i}, ragazz\textbf{i}, govern\textbf{o}, strad\textbf{a}, mett\textbf{ere}, bilanc\textbf{io}, aut\textbf{o}, mort\textbf{e}, giovan\textbf{i}

\item \textbf{Topic 15}: port\textbf{a}, figl\textbf{io}, mort\textbf{e}, lavor\textbf{o}, auto, ex, person\textbf{e}, ben\textbf{e}, madr\textbf{e}, eur\textbf{o}, poss\textbf{ono}, fer\textbf{iti};

\end{enumerate}

I tempi di esecuzione sono stati i seguenti:
\begin{itemize}
\item LDA con Spark:  2804 secondi;
\item LSI: 8 secondi
\end{itemize}
...
L'output è il seguente:
\begin{enumerate}
\item \textbf{Topic 1}: stat\textbf{o}, part\textbf{e}/part\textbf{ito}/part\textbf{ire}, ital\textbf{ia}, mil\textbf{a}/mil\textbf{ano}, paes\textbf{e}, govern\textbf{o}, eur\textbf{o}, milion\textbf{i}, pd, commission\textbf{e}, russ\textbf{ia}, città;

\item \textbf{Topic 2}: part\textbf{e}/part\textbf{ito}/part\textbf{ire}, govern\textbf{o}, pd, renz\textbf{i}, commission\textbf{e}, decision\textbf{e}, ue, vot\textbf{o}, sinistr\textbf{a}, accord\textbf{o}, may, pisapia;

   
\item \textbf{Topic 3}: ue, paes\textbf{e}, accord\textbf{o}, unit\textbf{i}, may, decision\textbf{e}, europe\textbf{a}, regn\textbf{o}, britann\textbf{ico}, union\textbf{e}, russ\textbf{ia}, merc\textbf{ato};
   
   
\item \textbf{Topic 4} eur\textbf{o}, ue, europe\textbf{a}, milion\textbf{i}, may, regn\textbf{o}, accord\textbf{o}, unit\textbf{i}, negoz\textbf{iati}, stat\textbf{o}, britann\textbf{ico}, premier;


\item \textbf{Topic 5}: mil\textbf{a}/mil\textbf{ano}, ital\textbf{ia}, men\textbf{o}, lavor\textbf{o}, paes\textbf{e}, figl\textbf{io}, diminu\textbf{zione}, rispett\textbf{o}, piazz\textbf{a}, pag\textbf{are}, alcun\textbf{i}, chiar\textbf{a};
   
\item \textbf{Topic 6}: ital\textbf{ia}, russ\textbf{ia}, piazz\textbf{a}, pd, mosc\textbf{a}, vot\textbf{o}, sinistr\textbf{a}, paes\textbf{e}, città, usa, palazz\textbf{o}, regol\textbf{e};
   
\item \textbf{Topic 7}: paes\textbf{e}, figl\textbf{io}, pd, parl\textbf{are}/parl\textbf{amento}, vit\textbf{a}, prezz\textbf{o}, alcun\textbf{i}, tagl\textbf{i}, dett\textbf{o}, russ\textbf{ia}, ius, legg\textbf{e};


\item \textbf{Topic 8}:  città, pass\textbf{ato}, piazz\textbf{a}, torin\textbf{o}, alcun\textbf{i}, part\textbf{e}/part\textbf{ito}, san, donn\textbf{e}, ospedal\textbf{e}, mond\textbf{o}, palazz\textbf{o}, appendin\textbf{o}\footnote{Riferito a Chiara Appendino, sindaco di Torino.};


\item \textbf{Topic 9}: milion\textbf{i}, città, paes\textbf{e}, mond\textbf{o}, ultim\textbf{i}, capital\textbf{e}, alcun\textbf{i}, parc\textbf{o}, sir\textbf{ia}, dollar\textbf{i}, punt\textbf{o}, prezz\textbf{o};


\item \textbf{Topic 10}: person\textbf{e}, parl\textbf{are}/parl\textbf{amento}, lavor\textbf{o}, altro, robot, dett\textbf{o}, vol\textbf{eva}/vol\textbf{i}, cont\textbf{o}, govern\textbf{o}, pass\textbf{ato}, ser\textbf{a}/ser\textbf{ie}, san;

\item \textbf{Topic 11}: president\textbf{e}, san, mil\textbf{a}/mil\textbf{ano}, pass\textbf{ato}, piazz\textbf{a}, acqua, berluscon\textbf{i}, ogni, ex, cert\textbf{o}, ser\textbf{a}/ser\textbf{ie}, sir\textbf{ia};

\item \textbf{Topic 12}: govern\textbf{o}, sir\textbf{ia}, unit\textbf{i}, possibil\textbf{ità}, comun\textbf{e}, ottobre, unic\textbf{o}, nord, colp\textbf{ito}, prezz\textbf{o}, camp\textbf{o}, centr\textbf{o};
  
\item \textbf{Topic 13}:   parl\textbf{are}/parl\textbf{amento}, bambin\textbf{i}, mond\textbf{o}, persone, città, italian\textbf{a}, milion\textbf{i}, cont\textbf{o}, lavor\textbf{o}, punt\textbf{o}, poch\textbf{hi}, giovan\textbf{i};


\item \textbf{Topic 14}: port\textbf{ato}, figl\textbf{io}, dett\textbf{o}, renz\textbf{i}, parl\textbf{are}/parl\textbf{amento}, madr\textbf{e}, famigl\textbf{ia}, piazz\textbf{a}, europe\textbf{a}, affid\textbf{ata}, organizz\textbf{azione}, ue

\item \textbf{Topic 15}: eur\textbf{o}, dett\textbf{o}, piazz\textbf{a}, russ\textbf{ia}, tagl\textbf{i}, san, donn\textbf{e}, mosc\textbf{a}, fond\textbf{o}, figl\textbf{io}, renz\textbf{i}, acqua.

\end{enumerate}

I tempi di esecuzione sono stati i seguenti:
\begin{itemize}
\item LDA con Spark:  525 secondi;
\item LSI: 2 secondi
\end{itemize}
...
L'output è il seguente:
\begin{enumerate}
\item \textbf{Topic 1}:  stat\textbf{o}, part\textbf{e}/part\textbf{ito}, pd, ital\textbf{ia}, punt\textbf{o}, eur\textbf{o}, figl\textbf{ia}, lavor\textbf{o}, renz\textbf{i}, person\textbf{e}, pass\textbf{ato}, berluscon\textbf{i};


\item \textbf{Topic 2}:  pd, part\textbf{e}/part\textbf{ito}, renz\textbf{i}, matte\textbf{o}, pers\textbf{o}, poss\textbf{ono}, pass\textbf{ato}, leader, elezion\textbf{i}, centrodestra, lavor\textbf{o}, ius

\item \textbf{Topic 3}: ser\textbf{ie}/ser\textbf{a}, punt\textbf{o}, mil\textbf{a}/mil\textbf{ano}, rom\textbf{a}, de, dett\textbf{o}, pag\textbf{are}, vip, sub\textbf{ito}, berluscon\textbf{i}, squadr\textbf{a}, camp\textbf{o}


\item \textbf{Topic 4} punt\textbf{o}, ital\textbf{ia}, ultim\textbf{a}, ogni, mar\textbf{ia}/mar\textbf{e}/mar\textbf{ito}, gioc\textbf{o}, nonost\textbf{ante}, davver\textbf{o}, berluscon\textbf{i}, bal\textbf{o}, talent\textbf{o}, europe\textbf{o}
   

\item \textbf{Topic 5}: part\textbf{e}/part\textbf{ito}, pass\textbf{ato}, qualc\textbf{he}, cert\textbf{o}, aul\textbf{a}, numer\textbf{o}, pens\textbf{a}, depress\textbf{i}, deput\textbf{ati}, padr\textbf{e}, sent\textbf{e}, ser\textbf{ie}/ser\textbf{a};

   
\item \textbf{Topic 6}: lavor\textbf{o}, spieg\textbf{a}, leader, raccont\textbf{a}, famigl\textbf{ia}, tass\textbf{e}, paes\textbf{e}, fragil\textbf{ità}, plag\textbf{iata}, sett\textbf{e}, showgirl, person\textbf{e};
   
   
\item \textbf{Topic 7}:  punt\textbf{o}, present\textbf{e}, provinc\textbf{ia}, davant\textbf{i}, vien\textbf{e}, tratt\textbf{a}, suon\textbf{o}, squadr\textbf{a}, già, alto, bolzano, entrat\textbf{e};
   
\item \textbf{Topic 8}:  alcun\textbf{i}, pap\textbf{a}, cardinal\textbf{e}, chies\textbf{a}, port\textbf{a}, magic\textbf{a}, mueller, ser\textbf{ie}/ser\textbf{a}, cerc\textbf{a}/cerc\textbf{hio}, scism\textbf{a}, berluscon\textbf{i}, vend\textbf{ita};
 
\item \textbf{Topic 9}: present\textbf{e},  spieg\textbf{a}, provinc\textbf{ia}, entrat\textbf{e}, ital\textbf{ia}, ser\textbf{ie}/ser\textbf{a}, alto, bolzano, riport\textbf{a}, numer\textbf{o}, riguard\textbf{a}, viv\textbf{ere};
  

\item \textbf{Topic 10}: provinc\textbf{ia}, giovan\textbf{e}, dat\textbf{o}/dat\textbf{a}, pubblic\textbf{o}, spos\textbf{ato}, bell\textbf{a}, alto, bolz\textbf{ano}, giudic\textbf{e}, ragazz\textbf{i}, famigl\textbf{ia}, rodriguez;


\item \textbf{Topic 11}: figl\textbf{ia}, padr\textbf{e}, pd, music\textbf{a}, cecilia, dev\textbf{e}, uomo, suon\textbf{o}, violin\textbf{o}, vien\textbf{e}, violenz, ignazio
   

   
\item \textbf{Topic 12}:  berlusconi, viene, music\textbf{a}, ital\textbf{ia}, vot\textbf{o}, rom\textbf{a}, port\textbf{a}, person\textbf{e}, suon\textbf{o}, violin\textbf{o}, silv\textbf{io}, però;


\item \textbf{Topic 13}:   rom\textbf{a}, scuol\textbf{a}, numer\textbf{o}, riport\textbf{a}, giovan\textbf{e}, napol\textbf{i}, bambin\textbf{i}, spos\textbf{ato}, nulla, stor\textbf{ia}, figl\textbf{ia}, davant\textbf{i}


\item \textbf{Topic 14}: figl\textbf{ia}, spos\textbf{ato}, giovan\textbf{e}, ragazz\textbf{i}, famigl\textbf{ia}, pd, padr\textbf{e}, dat\textbf{o}, entrat\textbf{e}, messagg\textbf{io}, consigl\textbf{io}, cent\textbf{o};
   
\item \textbf{Topic 15}: person\textbf{e}, nott\textbf{e}, provinc\textbf{ia}, poliz\textbf{ia}, alcun\textbf{i}, part\textbf{e}/part\textbf{ito}, davanti, spieg\textbf{are}, silv\textbf{io}, novembre, ogni, alto.
\end{enumerate}

I tempi di esecuzione sono stati i seguenti:
\begin{itemize}
\item LDA con Spark:  596 secondi;
\item LSI: 4 secondi
\end{itemize}
...
% Possiamo visualizzare le informazioni circa i tempi di esecuzione in funzione del numero di articoli (per l'LDA) e del numero di topic (per l'LSI) esaminati mediante il seguente grafico.

% Esecuzione
% Magari solo LDA

% Da esso si evince, anche se con pochi dati, una relazione di tipo lineare per il tempo di esecuzione. Le equazioni delle rette consentono di quantificare di quanto un articolo rallenta... 

% Può essere utile riguardare le relazioni rispetto non agli articoli o ai topic, ma al numero di parole  esaminate.

Per quanto riguarda l'output, osserviamo diversi fenomeni relativi ai topic:

\begin{itemize}
\item \emph{Primo cluster}: il primo cluster di parole ricopre un ruolo particolare, in quanto contiene i termini con il numero di occorrenze maggiore. Essa rappresenta una sorta di 'contenitore' delle parole-chiave dell'intero corpus. Al primo posto, per tutti, c'è la parola \emph{stato}, probabilmente è l'ambiguità di significato di questo termine che gli conferisce il numero spropositato di occorrenze che possiede. Potrebbe essere utile eliminare tale parola durante una precedente procedura di stemming;

\item \emph{Ripetizione}: molte parole sono ripetute in diversi topic  e, similmente, alcuni topic contigui possiedono quasi le stesse parole (es. \emph{Repubblica}: Topic 3 e 4).

\item \emph{Sovrapposizione}: alcuni topic contengono due diversi macroargomenti fusi al loro interno (es. \emph{La Stampa}: Topic 12 in cui sembra si fondano argomenti di politica estera e informazioni meteo d'attualità);

\item \emph{Interpretazione}: per alcuni topic è facile comprendere il tema di cui trattano (es. \emph{Corriere}: Topic 11, in cui si parla di cronache sportive), per altri non sembra possibile (es. \emph{Repubblica}: Topic 15). Alcuni topic accorpano parole che si rifanno a tematiche del tutto generali (es. \emph{Fanpage}: Topic 3, 7, 8 in cui emerge il tema della violenza sulle donne), altri entrano addirittura nello specifico di alcune particolari vicende (es. Repubblica: Topic 9, in cui è palese la vicenda della visita del Papa in Myanmar).

\end{itemize}

Il fenomeno del \emph{Primo Cluster} sono causati dalla riduzione che l'LSI opera. Ai primi posti dunque si collocano quei temi che spiegano una maggiore varianza (e che spesso hanno dunque un numero maggiore di occorenze). La \emph{Sovrapposizione} e la similitudine di topic contigui può essere causata dal fatto che abbiamo prestabilito la lunghezza delle parole nel topic, oppure, nel primo caso, dal fatto che due argomenti interferiscono magari per mezzo di una parole di collegamento. L'interpretazione è generalmente possibile anche se ci possono essere casi in cui non si evince facilmente (o non è possibile inferire) il tema centrale.

Dalla nostra analisi non supervisionata possiamo trarre alcune conclusioni.

Innanzitutto si evince lo schema giornalistico delle 3 'S', e si vede che i temi principali trattati sono , \emph{in primis}, politico-economici, e poi d'attualità e di cronaca (soprattutto nera e giudiziaria).

\emph{Repubblica} e \emph{Il Corriere}, testate di rilievo nazionale e le cui notizie raccolte sono un gran numero,  trattano di argomenti principalmente di matrice politico-economica. Non mancano, nella prima, vicende collegate all'attualità internazionale e all'istruzione, quest'ultimo tema presente anche nel \emph{Corriere} che, inoltre, possiede un topic di cronaca sportiva che non si trova negli altri giornali.

\emph{Il Giornale}, anch'esso di diffusione nazionale ma di cui abbiamo un minor numero di notizie a disposizione per l'analisi, è quasi interamente polarizzato verso la politica.

\emph{La Stampa}, di cui abbiamo un numero di notizie simile a quello de \emph{Il Giornale}, tratta di diverse tematiche, dalla politica all'attualità. Nel topic 8 si evince con sufficiente chiarezza la sua matrice territoriale.

Infine \emph{Fanpage}, possiede caratteristiche che spiccano nel contesto dei giornali analizzati. Ultimo per fondazione e con diffusione esclusivamente on-line, i temi trattati sono diversi: il tema politico è sempre ai primi posti, ma non è quello più affrontato. Principalmente le vicende di attualità, soprattutto cronaca nera, si contendono il gran numero di topic. Fa eccezione il topic 4, l'unico in tutte le testate in cui si evince nettamente che il tema trattato sono le informazioni metereologiche.
...
Si può voler definire un concetto di somiglianza tra quotidiani e verificare quali si assomigliano per tematiche trattate e quali, invece, sono  dissimili; si può volere, per ogni topic, assegnare un \emph{label} che ne indichi sinteticamente il contenuto, oppure si possono associare i topic a diverse aree tematiche (nel caso del giornalismo possono essere Politica, Economia, Cronaca, ecc.), cioè operare un'analisi \emph{supervisionata} dei topic. 

Il concetto di analisi supervisionata mediante LDA è stato affrontato ed esistono degli algoritmi, evoluzioni dell'LDA e in particolare l'sLDA \cite{Ble10}, in grado di operare una tale classificazione.

Anche il concetto di \emph{distanza}, tra due file testuali è stato sviluppato: mediante i topic estratti si viene a costituire una sorta di 'codice genetico' di ciascun documento e diversi documenti possono essere messi in relazione sulla base di un concetto appropriato di distanza\footnote{Tipicamente per l'LDA viene utilizzata la \emph{divergenza di Jensen-Shannon}, \cite{Sch99}} che misura le somiglianze tra i codici.

Un modo molto naive di assegnare un titolo sintetico a ciascun topic è quello di definire dei dizionari di parole-chiave (ad esempio si può associare \emph{Economia} a parole come 'lavoro', 'euro', 'milioni', ecc.), così facendo si può capire quali sono i temi trattati da ciascun topic e si può operare una sorta di intuitiva analisi supervisionata (anche se la dicitura è impropria perché un topic potrebbe possedere parole che appartengono a diverse classi).
...
\bibitem[Ble10]{Ble10} Blei David M., J. D. McAuliffe \emph{Supervised topic models},  Arxiv, 2010.
...
\bibitem[DeFin]{DeFin} B. de Finetti, \emph{De Finetti's Theorem}, \href{https://www.encyclopediaofmath.org/index.php?title=De_Finetti_theorem}{https://www.encyclopediaofmath.org/index.php?title=De\_Finetti\_theorem}, Encyclopedia of Mathematics.
...
\bibitem[GooN]{GooN} Google News, \href{https://news.google.com/news/}{https://news.google.com/news/}
...
\bibitem[Lin16]{Lin16} Lin Liu, Lin Tang, Wen Dong, Shaowen Yao and Wei Zhoucorresponding, \emph{An overview of topic modeling and its current applications in bioinformatics} \href{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5028368/
}{https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5028368/}, SpringerOpen, 2016
...
\bibitem[San17]{San17} Vicenda sottomarino \emph{San Juan}, \href{https://it.wikipedia.org/wiki/ARA_San_Juan_(S-42)}{https://it.wikipedia.org/wiki/ARA\_San\_Juan\_(S-42)},  2017.
...
\bibitem[Sch99]{Sch99} H. Schütze, C. D. Manning \emph{Foundations of Statistical Natural Language Processing}, MIT Press, 1999.
...
Ringrazio di cuore i miei i colleghi, in particolare Francesco Curia e Michele Cianfriglia, per avermi dato preziosi consigli durante questi anni, ma anche per i bei momenti insieme.
